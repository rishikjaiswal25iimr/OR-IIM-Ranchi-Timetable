...
- 20 Sessions per section (Hard)
- **Pre-Clustered Groups**: Sections are grouped to prevent internal clashes (Hard)
...
def solve_timetable(sections_df, init_rooms, red_rooms, red_week, max_daily):
    ...
    # We group sections into clusters that will share the EXACT same schedule.
    # Group size is limited by the smallest classroom bottleneck (reduced_rooms)
    ...
    # Constraint 4: Student Overlapping Across Groups (SOFT CONSTRAINT)
    tracked_overlaps = []
    for g1 in range(num_groups):
        for g2 in range(g1 + 1, num_groups):
...
with tab1:
    ...
    st.caption("Sections partitioned into the same 'Group' execute in parallel. This methodology ensures minimal student overlaps and massive improvements to solver runtime.")
    
    display_df = schedule_df.drop(columns=['Slot_Idx', 'Shift Category']).sort_values(by=["Week", "Day", "Slot", "Group", "Room"])
    # Moving 'Group' column to the front logically
    cols = ['Group', 'Course', 'Section', 'Faculty', 'Week', 'Day', 'Slot', 'Room']
...
with tab2:
    ...
    if not schedule_df.empty:
        ...
        filtered_course_df = filtered_course_df[['Group', 'Course', 'Section', 'Faculty', 'Week', 'Day', 'Slot', 'Room']]
        st.dataframe(filtered_course_df.sort_values(by=["Day", "Slot"]), use_container_width=True)
    else:
...
with tab3:
    ...
    if not schedule_df.empty:
        ...
        filtered_fac_df = filtered_fac_df[['Group', 'Course', 'Section', 'Faculty', 'Week', 'Day', 'Slot', 'Room']]
        st.dataframe(filtered_fac_df.sort_values(by=["Day", "Slot"]), use_container_width=True)
    else:
        st.info("No schedule data available.")
